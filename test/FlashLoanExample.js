...
const { DAI, DAI_WHALE, POOL_ADDRESS_PROVIDER } = require("../config/config.js");
...
describe("Deploy a Flash Loan", function () {
  it("Should take a flash loan and be able to return it", async function () {
    ...
    const BALANCE_AMOUNT_DAI = ethers.utils.parseEther("4");
    ...
    const tx = await _flashLoanExample.createFlashLoan(DAI, 1); // Borrow 1000 DAI in a Flash Loan with no upfront collateral
    await tx.wait();
